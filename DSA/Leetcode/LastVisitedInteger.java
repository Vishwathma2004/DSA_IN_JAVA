...
public class LastVisitedInteger {
    public List<Integer> lastVisitedInteger(int[] nums){
        ...
        for(int num:nums){
            if(num!=-1){
                ...
                if(count<=visited.size()){
                    ans.add(visited.get(visited.size()-count));
                }
                else{
                    ans.add(-1);
                }
    ...
    public static void main(String[] args) {
        ...
        LastVisitedInteger obj = new LastVisitedInteger();
        List<Integer> ans = obj.lastVisitedInteger(nums);
